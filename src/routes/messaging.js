...
const sendSchema = {
  body: {
    type: 'object',
    required: ['productCode', 'channel', 'templateCode', 'to'],
    properties: {
...
async function messagingRoutes(fastify, options) {
  ...
  fastify.get('/products/:code', async (request, reply) => {
    try {
      ...
      return reply.code(404).send({
        ...
        details: { productCode: request.params.code }
      });
    }
  });
  ...
  fastify.get('/dlq/stats', async (request, reply) => {
    const size = await redis.llen('herald:dlq');
    return reply.send({
      dlqSize: size,
  ...
  fastify.post('/dlq/reprocess', async (request, reply) => {
    ...
    for (let i = 0; i < limit; i++) {
      const message = await redis.rpop('herald:dlq');
      if (!message) break;
