...
class MessageService {
  ...
  async _sendToDLQ(data) {
    ...
    await redis.lpush('herald:dlq', JSON.stringify(dlqData));

    const dlqSize = await redis.llen('herald:dlq');
  ...
  /**
   * Check product rate limits (daily + per-minute)
   */
  async _checkRateLimit(productCode, dailyLimit) {
    // Check daily limit
    const today = new Date().toISOString().split('T')[0];
    const dailyKey = `ratelimit:${productCode}:${today}`;
    const dailyCount = await redis.get(dailyKey);
    const used = dailyCount ? parseInt(dailyCount, 10) : 0;

    if (used >= dailyLimit) {
      throw new RateLimitError(productCode, dailyLimit, used);
    }

    // Check per-minute limit (100 req/min per product)
    const minuteKey = `ratelimit:minute:${productCode}:${Math.floor(Date.now() / 60000)}`;
    const minuteCount = await redis.get(minuteKey);

    if (minuteCount && parseInt(minuteCount, 10) >= 100) {
      throw new RateLimitError(productCode, dailyLimit, used);
    }
  }

  /**
   * Increment product rate limit counters
   */
